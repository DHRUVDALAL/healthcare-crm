...
function escapeHtml(str) {
  ...
  async function loadApplicants() {
    ...
    els.tableBody.innerHTML = rows.map((a) => {
      ...
      const actions = `
        ...
        <button class="btn btn-primary btn-sm" data-action="followup" data-id="${a.id}" data-name="${escapeHtml(a.full_name)}">Follow-up</button>
  ...
  let followupCandidateId = null;

  function openFollowupModal(candidateId, candidateName) {
    followupCandidateId = candidateId;
    if (!followupModal) {
      followupModal = document.createElement('div');
      followupModal.id = 'followupModal';
      followupModal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.5);display:flex;align-items:center;justify-content:center;z-index:9999';
      followupModal.innerHTML = `
        <div style="background:var(--bg-primary);border-radius:12px;padding:28px;min-width:380px;max-width:90vw;box-shadow:var(--shadow-xl)">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px">
            <div style="font-weight:800;font-size:18px">Add Follow-up</div>
            <button id="closeFollowupModalBtn" style="border:none;background:none;font-size:20px;cursor:pointer;color:var(--text-secondary)">&times;</button>
          </div>
          <div id="followupCandidateName" style="color:var(--text-secondary);margin-bottom:16px;font-size:14px"></div>
          <form id="followupForm" style="display:grid;gap:12px">
            <div>
              <label style="font-size:13px;font-weight:600;margin-bottom:4px;display:block">Follow-up Date</label>
              <input type="date" id="followupDate" class="input" required />
            </div>
            <div>
              <label style="font-size:13px;font-weight:600;margin-bottom:4px;display:block">Follow-up Time</label>
              <input type="time" id="followupTime" class="input" />
            </div>
            <div>
              <label style="font-size:13px;font-weight:600;margin-bottom:4px;display:block">Mode</label>
              <select id="followupMode" class="select">
            ...
            <div>
              <label style="font-size:13px;font-weight:600;margin-bottom:4px;display:block">Outcome</label>
              <select id="followupOutcome" class="select">
            ...
            <div>
              <label style="font-size:13px;font-weight:600;margin-bottom:4px;display:block">Remarks</label>
              <textarea id="followupRemarks" class="input" rows="3" placeholder="Notes about this follow-up..."></textarea>
            </div>
            <div id="followupAlert" style="color:var(--danger);font-size:13px;display:none"></div>
            <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:4px">
              <button type="button" id="cancelFollowupBtn" class="btn btn-outline">Cancel</button>
      ...
      document.getElementById('closeFollowupModalBtn').addEventListener('click', closeFollowupModal);
    ...
    document.getElementById('followupCandidateName').textContent = `Candidate: ${candidateName}`;
  ...
  function closeFollowupModal() {
    if (followupModal) followupModal.style.display = 'none';
  ...
  async function submitFollowup(e) {
    ...
    const dateVal = document.getElementById('followupDate').value;
    if (!dateVal) {
      alertEl.textContent = 'Follow-up date is required.';
      alertEl.style.display = 'block';
      return;
    }
    try {
      await window.CRM_API.request('/api/follow-ups', {
        method: 'POST',
        body: JSON.stringify({
          applicant_id: followupCandidateId,
          follow_up_date: dateVal,
          follow_up_time: document.getElementById('followupTime').value || null,
          mode: document.getElementById('followupMode').value,
          outcome: document.getElementById('followupOutcome').value,
          remarks: document.getElementById('followupRemarks').value.trim()
        })
      });
      closeFollowupModal();
      alert('Follow-up logged successfully!');
    } catch (err) {
      alertEl.textContent = err.message || 'Failed to save follow-up.';
      alertEl.style.display = 'block';
    }
  }
  ...
  function bindEvents() {
    ...
    els.tableBody.addEventListener('click', async (e) => {
      ...
      if (action === 'view-resume') return viewResume(id);
      if (action === 'download-resume') return downloadResume(id);
      if (action === 'followup') return openFollowupModal(id, btn.dataset.name || '');
    });
  ...
  async function init() {
    try {
      ...
      console.warn('[MyCandidates] init warning:', err.message);
      if (err.status === 401) {
        window.CRM_API.clearToken();
        window.location.href = './login.html';
      }
